...
class Kaprodi extends CI_Controller
{
    ...
    public function editProfileKaprodi($id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Proses update data
            $updateData = $this->Kelola_user_model->updateDataProfileTtd($id);
            ...
            if (empty($this->input->post('passnow'))) {
                ...
                $updatePassword = $this->Kelola_user_model->updateDataProfileTtdPassword($id);
    ...
    public function _rulesStatus()
    {
        ...
        // Jika status ditolak
        if ($status == 'Ditolak Kaprodi') {
            if (empty($this->input->post('alasan_kaprodi', true))) {
                $this->form_validation->set_rules('alasan_kaprodi', 'Alasan ditolak', 'trim|required', ['required' => '%s harus diisi!']);
            }
        }
